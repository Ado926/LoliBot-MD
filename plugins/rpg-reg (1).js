...
${nnaa}`, contextInfo: { forwardedNewsletterMessageInfo: { newsletterJid: ['120363402846939411@newsletter', '120363402846939411@newsletter'].getRandom(), serverMessageId: '', newsletterName: 'Abundantes Botsitos 👻' }, forwardingScore: 9999999, isForwarded: true, "externalAdReply": {"showAdAttribution": true, "containsAutoReply": true, "title": `𝐑𝐄𝐆𝐈𝐒𝐓𝐑𝐎 𝐂𝐎𝐌𝐏𝐋𝐄𝐓𝐀𝐃𝐎`, "body": wm, "previewType": "PHOTO", thumbnail: img.getRandom(), sourceUrl: [nna, nna2, nn, md, yt, tiktok].getRandom()}}}, { quoted: fkontak, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100 });
//await m.reply(`${sn}`);
let ppch = await conn.profilePictureUrl(who, 'image').catch(_ => imageUrl.getRandom()) 
await global.conn.sendMessage(global.ch.ch1, { text: `◉ *${await tr("Usuario")}:* ${m.pushName || 'Anónimo'}  ${userNationality ? `\n◉ *${await tr("País")}:* ${userNationality}` : ''}
